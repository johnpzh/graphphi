		...
		int *is_updating_active_side)
{
	...
	for (unsigned edge_i = 0; edge_i < bound_edge_i; edge_i += NUM_P_INT) {
		...
		__m512i tail_v = _mm512_mask_loadu_epi32(_mm512_undefined_epi32(), is_active_m, tails_buffer + edge_i);
...
int main( int argc, char** argv) 
{
	...
	print_benchmark_head("bfs", input_f);
	...
	for (unsigned i = 6; i < run_count; ++i) {
		...
		double wom_mean = bot_best_perform.get_mean();
		// W/ Merge
		puts("W/ Merge:");
		bot_best_perform.reset();
		for (unsigned k = 0; k < k_set; ++k) {
		...
		bot_best_perform.print_average(NUM_THREADS);
		double wm_mean = bot_best_perform.get_mean();
