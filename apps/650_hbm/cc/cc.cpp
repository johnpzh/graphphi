...
int main(int argc, char *argv[]) 
{
	...
	if (argc > 2) {
		...
		//TILE_WIDTH = 1024;
		//ROW_STEP = 16;
		printf("Usage: ./cc <data> <tile_width> <stripe_length>\n");
		exit(1);
	}
	print_benchmark_head("cc", filename);
...
//#endif
	...
	for (unsigned i = 6; i < run_count; ++i) {
		...
		// write to file
		double mean = bot_best_perform.get_mean();
		bot_best_perform.write_to("mean.txt", "MEAN");
	}
