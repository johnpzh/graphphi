...
inline void update_dense_serial(
...
inline unsigned update_sparse_serial(
...
unsigned *BFS_kernel_sparse_serial(
...
//		unsigned offset = degrees[i];
//		unsigned size = 0;
//		unsigned bound_edge_i = graph_vertices[start] + graph_degrees[start];
...
#pragma omp parallel for num_threads(64)
	...
	unsigned *new_frontier = BFS_sparse_serial(
								frontier,
								graph_vertices,
								graph_edges,
								graph_degrees,
...
#pragma omp parallel for reduction(+: out_degree)
	...
	while (true) {
		if (frontier_size + out_degree > bfs_threshold) {
			...
			new_frontier = BFS_sparse_serial(
								frontier,
								graph_vertices,
								graph_edges,
								graph_degrees,
		...
		if (last_is_dense) {
			...
			out_degree = update_sparse_serial(
								frontier,
								frontier_size,
								graph_degrees,
								h_graph_parents,
...
inline unsigned update_sparse_simd(
				unsigned *h_graph_queue,
				const unsigned &queue_size,
				unsigned *graph_degrees,
				unsigned *h_graph_parents,
...
#pragma omp parallel for num_threads(64)
	...
	//// End PAPI
	double start_time = omp_get_wtime();
	unsigned *new_frontier = BFS_sparse_simd(
								frontier,
								graph_vertices,
								graph_edges,
								graph_degrees,
...
int main( int argc, char** argv) 
{
	...
	print_benchmark_head("bfs", filename);
