...
inline void update_dense_weighted_serial(
...
inline void sssp_kernel_dense_weighted_serial(
				unsigned *graph_heads, 
				unsigned *graph_tails, 
				unsigned *graph_weights,
				int *graph_active, 
...
inline void scheduler_dense_weighted_serial(
					unsigned *graph_heads, 
					unsigned *graph_tails, 
					unsigned *graph_weights,
					unsigned *tile_offsets,
...
#pragma omp parallel for schedule(dynamic, 1)
	for (unsigned tile_index = start_tile_id; tile_index < end_tile_id; tile_index += tile_step) {
		...
		for (unsigned tile_id = tile_index; tile_id < bound_tile_id; ++tile_id) {
			...
			sssp_kernel_dense_weighted_serial(
				graph_heads, 
				graph_tails, 
				graph_weights,
				graph_active, 
...
inline void BFS_dense_weighted_serial(
					unsigned *graph_heads,
					unsigned *graph_tails,
					unsigned *graph_weights,
					unsigned *tile_offsets,
					...
					unsigned *dists)
{
	...
	for (unsigned side_id = 0; side_id < bound_side_id; side_id += ROW_STEP) {
		scheduler_dense_weighted_serial(
				graph_heads, 
				graph_tails, 
				graph_weights,
				tile_offsets,
...
inline unsigned *to_sparse_serial(
		int *h_graph_mask,
		const unsigned &frontier_size)
{
...
inline unsigned sparse_update_weighted_serial(
				unsigned *h_graph_queue,
...
inline unsigned *BFS_kernel_sparse_weighted_serial(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_weights_csr,
				unsigned *graph_degrees,
...
inline unsigned *BFS_sparse_weighted_serial(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
				...
				unsigned *dists)
{
	return BFS_kernel_sparse_weighted_serial(
				graph_vertices,
				graph_edges,
				graph_weights_csr,
				graph_degrees,
		...
		const unsigned &source)
{
	...
	unsigned *new_queue = BFS_sparse_weighted_serial(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
	...
	out_degree =  sparse_update_weighted_serial(
							h_graph_queue,
							frontier_size,
							graph_degrees,
							h_updating_graph_mask);
	...
	while (true) {
		if (frontier_size + out_degree > pattern_threshold) {
			...
			// Sparse
			if (last_is_dense) {
				new_queue = to_sparse_serial(
							h_graph_mask,
							frontier_size);
				free(h_graph_queue);
				h_graph_queue = new_queue;
			}
			new_queue = BFS_sparse_weighted_serial(
									h_graph_queue,
									frontier_size,
									graph_vertices,
									graph_edges,
...
inline void update_dense_weighted_simd(
					unsigned &_frontier_size,
					unsigned &_out_degree,
					int *h_graph_mask,
					int *h_updating_graph_mask,
...
inline void sssp_kernel_dense_weighted_simd(
...
inline void scheduler_dense_weighted_simd(
					unsigned *heads_buffer,
					unsigned *tails_buffer,
					unsigned *weights_buffer,
					unsigned *graph_heads, 
		...
		for (unsigned tile_id = tile_index; tile_id < bound_tile_id; ++tile_id) {
			...
			while (remain != 0) {
				if (capacity > 0) {
					...
					sssp_kernel_dense_weighted_simd(
							heads_buffer_base,
							tails_buffer_base,
							weights_buffer_base,
							size_buffer,
...
inline void BFS_dense_weighted_simd(
					unsigned *graph_heads,
					unsigned *graph_tails,
					unsigned *graph_weights,
					unsigned *tile_offsets,
...
inline unsigned update_sparse_weighted_simd(
				unsigned *h_graph_queue,
				const unsigned &queue_size,
				unsigned *graph_degrees,
				int *h_updating_graph_mask)
...
inline unsigned *BFS_kernel_sparse_weighted_simd(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_weights_csr,
				unsigned *graph_degrees,
...
inline unsigned *BFS_sparse_weighted_simd(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
				...
				unsigned *dists)
{
	return BFS_kernel_sparse_weighted_simd(
				graph_vertices,
				graph_edges,
				graph_weights_csr,
				graph_degrees,
		...
		const unsigned &source)
{
	...
	h_graph_queue[0] = source;
	unsigned *new_queue = BFS_sparse_weighted_simd(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
	...
	last_is_dense = false;
	// Get the sum of the number of active nodes and their out degrees
	out_degree =  update_sparse_weighted_simd(
							h_graph_queue,
							frontier_size,
							graph_degrees,
							h_updating_graph_mask);
	...
	while (true) {
		if (frontier_size + out_degree > pattern_threshold) {
			...
			// Sparse
			if (last_is_dense) {
				new_queue = to_sparse_simd(
							h_graph_mask,
							frontier_size);
				_mm_free(h_graph_queue);
				h_graph_queue = new_queue;
			}
			new_queue = BFS_sparse_weighted_simd(
									h_graph_queue,
									frontier_size,
									graph_vertices,
									graph_edges,
