...
inline unsigned update_visited_serial(
		int *h_graph_mask,
		int *h_graph_visited)
{
	unsigned count = 0;
...
inline void update_visited_reverse_serial(
		int *h_graph_mask,
		int *h_graph_visited,
		float *dependencies,
...
///////////////////////////////////////////////////////
// Sparse
void update_visited_sparse_reverse_serial(
...
inline unsigned *BFS_kernel_sparse_serial(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_degrees,
				int *h_graph_visited,
...
inline unsigned *BFS_sparse_serial(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
				...
				unsigned *num_paths)
{
	return BFS_kernel_sparse_serial(
				graph_vertices,
				graph_edges,
				graph_degrees,
				h_graph_visited,
...
inline void BFS_kernel_sparse_reverse_serial(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_degrees,
				int *h_graph_visited,
...
inline void BFS_sparse_reverse_serial(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
				...
				float *dependencies)
{
	BFS_kernel_sparse_reverse_serial(
				graph_vertices,
				graph_edges,
				graph_degrees,
				h_graph_visited,
...
void update_visited_dense_reverse_serial(
		unsigned *h_graph_mask,
		int *h_graph_visited,
		int *is_active_side,
		float *dependencies,
...
inline void bfs_kernel_dense_serial(
...
inline void scheduler_dense_serial(
...
inline unsigned *BFS_dense_serial(
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
		unsigned *h_graph_mask,
...
inline void bfs_kernel_dense_reverse_serial(
		const unsigned &start_edge_i,
		const unsigned &bound_edge_i,
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
...
inline void scheduler_dense_reverse_serial(
		const unsigned &start_row_index,
		const unsigned &tile_step,
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
...
#pragma omp parallel for schedule(dynamic, 1)
	for (unsigned tile_index = start_tile_id; tile_index < end_tile_id; tile_index += tile_step) {
		...
		for (unsigned tile_id = tile_index; tile_id < bound_tile_id; ++tile_id) {
			...
			bfs_kernel_dense_reverse_serial(
					tile_offsets[tile_id],
					bound_edge_i,
					h_graph_heads,
					h_graph_tails,
...
inline void BFS_dense_reverse_serial(
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
		unsigned *h_graph_mask,
		//int *h_updating_graph_mask,
		...
		float *dependencies)
{
	...
	for (unsigned side_id = 0; side_id < bound_side_id; side_id += ROW_STEP) {
		scheduler_dense_reverse_serial(
				//side_id * SIDE_LENGTH,\
				//(side_id + ROW_STEP) * SIDE_LENGTH,
				side_id,
				ROW_STEP,
		...
		const unsigned &source)
{
	...
	// First Phase
	unsigned *new_queue = BFS_sparse_serial(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
	...
	while (true) {
		if (frontier_size + out_degree > bfs_threshold) {
			if (!last_is_dense) {
				...
				h_graph_mask = to_dense_serial(
					is_active_side,
					h_graph_queue,
					frontier_size);
			}
			unsigned *new_mask = BFS_dense_serial(
					graph_heads,
					graph_tails,
					h_graph_mask,
					//h_updating_graph_mask,
			...
			// Sparse
			if (last_is_dense) {
				h_graph_queue = to_sparse_serial(
					frontier_size,
					h_graph_mask);
			}
			new_queue = BFS_sparse_serial(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
...
#pragma omp parallel for num_threads(NUM_THREADS)
	...
	for (int lc = level_count - 1; lc >= 0; --lc) {
		...
		if (is_dense_frontier[lc]) {
			update_visited_dense_reverse_serial(
										frontier,
										h_graph_visited,
										is_active_side,
										dependencies,
										inverse_num_paths);
			BFS_dense_reverse_serial(
					graph_heads_reverse,
					graph_tails_reverse,
					frontier,
					h_graph_visited,
			...
			update_visited_sparse_reverse_serial(
										frontier,
										queue_size,
										h_graph_visited,
										dependencies,
										inverse_num_paths);
			BFS_sparse_reverse_serial(
							frontier,
							queue_size,
							graph_vertices_reverse,
							graph_edges_reverse,
...
///////////////////////////////////////////////////////
// Sparse
inline unsigned update_visited_sparse_simd(
						int *h_graph_visited,
						unsigned *h_graph_queue,
						unsigned queue_size,
						unsigned *graph_degrees)
...
inline void update_visited_sparse_reverse_simd(
		unsigned *h_graph_queue,
		const unsigned &queue_size,
		int *h_graph_visited,
		float *dependencies,
...
inline unsigned *to_sparse_simd(
		const unsigned &frontier_size,
		unsigned *h_graph_mask)
{
	//unsigned *new_frontier = (unsigned *) malloc(frontier_size * sizeof(unsigned));
...
inline unsigned *BFS_kernel_sparse_simd(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_degrees,
				int *h_graph_visited,
...
inline unsigned *BFS_sparse_simd(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
...
inline void BFS_kernel_sparse_reverse_simd(
				unsigned *graph_vertices,
				unsigned *graph_edges,
				unsigned *graph_degrees,
				int *h_graph_visited,
...
inline void BFS_sparse_reverse_simd(
				unsigned *h_graph_queue,
				unsigned &queue_size,
				unsigned *graph_vertices,
				unsigned *graph_edges,
				...
				float *dependencies)
{
	BFS_kernel_sparse_reverse_simd(
				graph_vertices,
				graph_edges,
				graph_degrees,
				h_graph_visited,
...
inline void update_visited_dense_simd(
...
void update_visited_dense_reverse_simd(
		unsigned *h_graph_mask,
		int *h_graph_visited,
		int *is_active_side,
		float *dependencies,
...
inline void bfs_kernel_dense_simd(
...
inline void scheduler_dense_simd(
		const unsigned &start_row_index,
		const unsigned &tile_step,
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
...
inline unsigned *BFS_dense_simd(
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
		unsigned *h_graph_mask,
		//int *h_updating_graph_mask,
...
inline void bfs_kernel_dense_reverse_simd(
		unsigned *heads_buffer,
		unsigned *tails_buffer,
		const unsigned &size_buffer,
		unsigned *h_graph_mask,
...
inline void scheduler_dense_reverse_simd(
		const unsigned &start_row_index,
		const unsigned &tile_step,
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
...
inline void BFS_dense_reverse_simd(
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
		unsigned *h_graph_mask,
		//int *h_updating_graph_mask,
		...
		const unsigned &source)
{
	...
	double start_time = omp_get_wtime();
	// First Phase
	unsigned *new_queue = BFS_sparse_simd(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
	...
	// Update the h_graph_visited, get the sum of the number of active nodes and their out degrees.
	unsigned out_degree = update_visited_sparse_simd(
											h_graph_visited,
											h_graph_queue,
											frontier_size,
											graph_degrees);
	...
	while (true) {
		if (frontier_size + out_degree > bfs_threshold) {
			if (!last_is_dense) {
				free(is_active_side);
				is_active_side = (int *) calloc(NNODES, sizeof(int));
				h_graph_mask = to_dense_simd(
					is_active_side, 
					h_graph_queue, 
					frontier_size);
			}
			unsigned *new_mask = BFS_dense_simd(
					graph_heads,
					graph_tails,
					h_graph_mask,
					//h_updating_graph_mask,
			...
			// Sparse
			if (last_is_dense) {
				h_graph_queue = to_sparse_simd(
					frontier_size,
					h_graph_mask);
			}
			new_queue = BFS_sparse_simd(
								h_graph_queue,
								frontier_size,
								graph_vertices,
								graph_edges,
		...
		if (last_is_dense) {
			...
			out_degree = update_visited_sparse_simd(
										h_graph_visited,
										h_graph_queue,
										frontier_size,
										graph_degrees);
...
//	}
	...
	for (int lc = level_count - 1; lc >= 0; --lc) {
		frontier = frontiers[lc];
		if (is_dense_frontier[lc]) {
			update_visited_dense_reverse_simd(
										frontier,
										h_graph_visited,
										is_active_side,
										dependencies,
										inverse_num_paths);
			BFS_dense_reverse_simd(
					graph_heads_reverse,
					graph_tails_reverse,
					frontier,
					h_graph_visited,
			...
			unsigned queue_size = frontier_sizes[lc];
			update_visited_sparse_reverse_simd(
										frontier,
										queue_size,
										h_graph_visited,
										dependencies,
										inverse_num_paths);
			BFS_sparse_reverse_simd(
							frontier,
							queue_size,
							graph_vertices_reverse,
							graph_edges_reverse,
